...
create table if not exists thoth.files (
  id text primary key default gen_random_uuid()::text,
...
create table if not exists thoth.message_files (
...
create index if not exists message_files_message_position_idx
  on thoth.message_files (message_id, attachment_position asc);

create index if not exists message_files_file_id_idx
